...
for gw in "${CANDIDATES[@]}"; do
    ...
    # Step 4: Run speedtest (downstream in Mbps)
    SPEED=$(speedtest-cli --secure --simple 2>/dev/null | awk '/Download/{print $2}')
    if [ -z "$SPEED" ]; then
        echo "⚠️  internet + DNS OK ($LAT ms) but speedtest failed"
        continue
    fi

    echo "✅ works ($LAT ms, ${SPEED} Mbps download😘)"

    # Track fastest speed (instead of lowest latency)
...
# Final decision after testing all gateways
if [ -n "$BEST_GATEWAY" ]; then
    echo ""
    echo "🎯 Best working gateway: $BEST_GATEWAY ($BEST_LAT ms, ${BEST_SPEED} Mbps)"
